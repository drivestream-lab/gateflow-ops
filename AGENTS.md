...
Agent skills: **`prayog-skills/`** (git submodule at root, pinned at **v0.5.1**) — `/spec-draft`, `/initiative-feasibility`, `/spec-technical-review`, `/spec-implementation-plan`, `/pre-implement`, `/loop-spec`, `/learning-extract`, `/ground-spec`, `/purge-initiative-artifacts-app`, `/commit-workspace`, `/open-draft-pr`, `/create-board-tickets`.
