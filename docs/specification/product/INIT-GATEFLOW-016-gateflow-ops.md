...
platform-admin wipe / lane-defaults / agent-catalogue surfaces; Launchpad
scaffolding; `prayog-skills` pin/dispatch edits; auto-lift/auto-merge;
client-invented capability tiers beyond JWT role from gateflow.
...
| REQ-04 | Connect or re-sync the tenant’s programme meta connection before catalogue use            | PRD `REQ-04`, `CAP-B`          | Operator connects/re-syncs programme                               | Connection put/get succeed; catalogue browsing is gated on connection being available. Until gateflow auto-connects on programme create (Q-4), one-time connect with meta org/repo may still be required after attach. | live verify               |
| REQ-05 | Admit a catalogue candidate into the fleet and show per-repo outcome in plain language    | PRD `REQ-05`, `CAP-B`          | Operator admits a candidate                                        | Select response outcome (`ok` / `already_selected` / `setup_failed` / `status_failed` / `probe_failed` / `out_of_catalogue`) is shown as operator-readable text, not a raw enum. Admit = fleet onboard + harness prep — **not** wave start (CAP-C). | live verify               |
...
| REQ-07 | Compose a single pass/fail **fleet onboard** verdict client-side from select + readiness signals | PRD `REQ-07`, `CAP-B`, D7/OQ-2 | REQ-05 and REQ-06 results are available                            | UI shows exactly one pass/fail presentation — never a partial/ambiguous membership state; verdict is fleet readiness, not wave execution                                      | unit + live verify        |
...
| REQ-32 | `platform_admin` can list onboarded programmes                                            | Backfill `CAP-P`               | Platform admin opens programmes list                               | Programme list from gateflow `GET /api/v1/programmes` renders (empty state allowed)                                                                                             | live verify               |
| REQ-33 | `platform_admin` can validate-then-create a programme                                     | Backfill `CAP-P`               | Platform admin submits onboard form (name, meta org/repo, PAT) — no workspace or `meta_ref` field | Programme + child tenant created only after gateflow validation; success navigates to detail; PAT never shown on read; workspace path may appear on detail from gateflow read model (host `GATEFLOW_WORKSPACE_ROOT`); ops does not collect or forward `meta_ref` | live verify               |
| REQ-34 | `platform_admin` can open programme detail                                                | Backfill `CAP-P`               | Platform admin opens a programme                                   | Detail shows programme read model (id, name, tenant_id, meta, workspace) plus informational `repo_catalogue` candidates when present — not fleet membership; without secrets | live verify               |
| REQ-37 | `platform_admin` can refresh programme repo catalogue from meta                           | Backfill `CAP-P` (gateflow REQ-49) | Platform admin triggers catalogue refresh on programme detail   | Snapshot rewritten from meta; UI updates candidates; no Fleet connect required; fail-closed on git/parse errors                                                              | live verify               |
| REQ-35 | `platform_admin` can attach a `tenant_admin` for a programme                              | Backfill `CAP-P`               | Platform admin submits credential_identifier + password            | Attach succeeds; UI shows success without exposing minted `access_token`; that identity can later log in as tenant_admin                                                        | live verify               |
| REQ-36 | Primary nav is filtered by JWT role                                                       | Backfill `CAP-P`               | Signed-in operator views shell                                     | `platform_admin` sees Status + Programmes; `tenant_admin` sees Status + Tenant + Fleet; cross-role pages redirect or show honest empty                                          | unit + live verify        |

> **Id convention:** `REQ-*` canonical. `REQ-01`–`REQ-31` match PRD. `REQ-32`–
> `REQ-37` are CAP-P backfill (platform programme registry) added in-repo.
...
- Nav is filtered by gateflow-issued JWT `role` (`platform_admin` vs `tenant_admin`) — no client-invented capability matrix beyond that (amends PRD D3 for CAP-P).
- CAP-P may call `PLATFORM_ADMIN`-gated programme list/create/get/tenant-admins only. Wipe, lane-defaults, and agent-catalogue remain out of scope.
- Process/workflow editing is display-only; pin/`dispatch` remain outside this console (PRD D5).
- GitHub appears as outbound links only — no direct GitHub write from this portal (PRD §4). Attach responses must not expose upstream `access_token` to the browser.
...
| REQ-32–REQ-35     | Non-`platform_admin` calls CAP-P BFF                                                               | Refused (403); no programme create/attach                                  | Prevents role leakage                  | live verify / unit       |
| REQ-35            | Attach response includes upstream `access_token`                                                   | BFF strips token before JSON to browser                                    | Prevents JWT leakage to client storage | unit + inspection        |
| REQ-33            | Validation failure (bad PAT / meta / non-HTTPS catalogue links)                                    | Named failure; no durable programme presented as success                   | Fail-closed create                     | live verify              |
| REQ-33            | Ops create body includes `workspace_root`, `meta_ref`, or agent key fields                 | Must not be collected or forwarded — workspace is host env; `meta_ref` omitted by ops UX | Align with gateflow onboard contract   | inspection + live verify |
| REQ-37            | Catalogue refresh git/parse failure                                                                | Named failure; prior `repo_catalogue` unchanged; UI shows error            | Fail-closed refresh                    | live verify              |
...
- Platform-admin wipe, lane-defaults, and agent-catalogue surfaces (programme list/create/detail/attach only for CAP-P)
...
- Auto-lift checkpoints / auto-merge; process editing in UI
- Cross-tenant tenant-list browsing
- Auto session-switch to attached `tenant_admin` JWT after attach
- Auto-create tenant `programme_connection` on programme create (gateflow ask Q-4 — consumer waits on upstream; CAP-B keeps manual connect until then)
- Wave start / run / forge / initiative co-pilot UI on the Fleet page (CAP-C / CAP-F remain later waves; fleet select only admits repos)
...
| CTR-02      | gateflow / PE    | gateflow-ops / PE | CAP-B programme connect, catalogue, select/deselect, readiness refresh           | Tenant id; programme connect (`org`/`repo` only — no `ref`); candidate selection; readiness refresh | Connection; catalogue; per-repo outcomes; harness_verified + verdict_type | Outcomes enumerated; fail outcomes block membership; select ≠ wave start (CAP-C) | Structured fail outcomes; upstream errors surfaced | Unchanged; composition stays client-side (REQ-07)     | unit (composition) + live verify        |
...
| CTR-08      | gateflow / PE    | gateflow-ops / PE | CAP-P platform programmes: list, create, get, catalogue refresh, attach tenant_admin | Create: `name`, `meta_org`, `meta_repo`, `github_pat` (no `workspace_root` / `meta_ref` / agent keys); programme id; attach credentials; catalogue refresh | Programme list/detail include `repo_catalogue` + `workspace_root`; create/refresh result; attach ack without access_token | `platform_admin` only; PAT never on read; attach token stripped; workspace from gateflow `GATEFLOW_WORKSPACE_ROOT`; catalogue informational (≠ fleet); refresh fail-closed; meta HTTPS-only | Auth/role mismatch; validation failures | Aligned to gateflow REQ-48/49; ops omits optional upstream `meta_ref` | unit (strip/nav/map) + live verify          |

## Non-functional requirements

| Area                      | Requirement or N/A rationale                                                                                                                                       | Acceptance / evidence                                      |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------- |
| Security                  | Session cookie stays httpOnly; upstream JWT only on server BFF; no browser storage of upstream tokens; CAP-P strips attach `access_token`; forge only on explicit authorize | inspection + live verify; route audit                      |
...
| A-6 | Gateflow programme create does not accept client `workspace_root`; ops CAP-P must not send it; workspace is host `GATEFLOW_WORKSPACE_ROOT` | gateflow `ProgrammeOnboardRequest` + orchestration settings | PE | confirmed (upstream) | Gateflow reintroduces client-supplied workspace on create |
| A-7 | Ops does not collect `meta_ref` / connect `ref`; gateflow treats both as optional (default remote HEAD) | gateflow programme + connect DTOs | PE | confirmed (upstream) | Product requires pinned-ref UX again |
...
| Q-4 | PE   | **Gateflow ask:** On successful `POST /api/v1/programmes`, auto-upsert `tenant_programme_connections` for the child tenant from programme `meta_org`/`meta_repo` (git already resolved at create) so attach → login → Fleet catalogue works without re-typing meta | gateflow PE | no | CAP-B tenant UX after CAP-P | Keep one-time manual connect in Mission Control CAP-B until upstream ships | open | raised from ops CAP-P/CAP-B rethink |

**Paste-ready gateflow ask (Q-4):** Auto-create tenant programme connection on Programme validate-then-create. Problem: Mission Control tenant Fleet requires manual `PUT …/connect` with meta org/repo even though Programme already stored that meta and cloned it at create. Ask: On successful programme create, upsert `programme_connection` for the child tenant from programme `meta_*`. Acceptance: After create + attach tenant_admin + login, `GET /api/v1/tenants/{tenant_id}/programme/connection` returns 200 without a prior connect call; catalogue/select work with existing PAT/workspace.

## Draft check summary (D1–D12)

| Check                         | Status | Evidence / findings                                                                                                                                                                                                                                                                              |
| ----------------------------- | ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| D1 Approved handoff current   | PASS   | Head `5422e0f…` = APPROVED review `commit_id`; H1 digest match; H3 rev 1; H2 scope digest present; repo affected; label `impact-map-lgtm`. Note: review _body_ text still cites `meta_pr_head_sha: 003e4228…` (prior commit); PRD+map bytes identical on both SHAs — formal G1 uses `commit_id`. |
...
| D6 Assumptions/questions      | PASS   | A-1–A-7; Q-1–Q-4 non-blocking with defaults                                                                                                                                                                                                                                                      |
...
- Open engineering questions: Q-1–Q-4 (non-blocking; Q-4 is gateflow auto-connect ask)
