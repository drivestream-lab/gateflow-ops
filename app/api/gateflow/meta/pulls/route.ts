// INIT-019 W0 — list programme meta PRs from gateflow. Never call GitHub here.
...
export async function GET(request: NextRequest) {
  ...
  const logger = createApiLogger(request.method, request.url, correlationId, {
    module: "gateflow-meta-pulls-api",
  ...
  const upstreamPath = refresh
    ? `/api/v1/tenants/${tenantId}/programme/meta/pulls?refresh=true`
    : `/api/v1/tenants/${tenantId}/programme/meta/pulls`;

  try {
    const res = await upstreamFetch(upstreamPath, {
      correlationId,
    });
    if (!res.ok) {
